...
class Estimator(Counter):
	...
	def getNgramsCount(self, nRange = 2, returnType = "dict", countDone = False, exact = False, lc = True, uc = False, includeNumbers = True, includePunctuation = False):
		ngramsList = []
	...
	def getNgramsFrequencies(self, ngramsList = None, nRange = 2, returnType = "dict", countDone = False, exact = False, lc = True, uc = False, includeNumbers = True):
		...
		def getTotalCount(ngramsList):
			#total = 0
			if nRange == 2:
				#	for key1 in ngramsList.keys():
				#		for key2 in ngramsList[key1].keys():
				#			total += ngramsList[key1][key2]
				#return total
				return sum([ngramsList[key1][key2] for key1 in ngramsList.keys() for key2 in ngramsList[key1].keys()])

		print getTotalCount(ngramsList)
		#print ngramsFrequenciesList['pisikal'], total
...
if __name__ == "__main__":
	...
	print "getNgramsCount()\t\t\t", e.getNgramsCount()
