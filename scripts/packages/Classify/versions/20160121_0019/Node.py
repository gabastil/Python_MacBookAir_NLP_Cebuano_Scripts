...
class Node(ObjectSet.ObjectSet):
	...
	def __init__(self, examples = None, attributes = None, branchCount = 2):
	...
	def entropyA(self, examples):
		...
		#print examples.getDistribution(), self.entropy(examples.getDistribution())
	...
	def entropyB(self, examples, attribute, branchCount):
		...
		#print 'a', sum([])*1., 'c', c, 'cc', [[v/sum(branch) for v in branch] for branch in c if sum(branch) > 0.]
	...
	def informationGain(self, examples, attribute, branchCount):
		...
		#print 'a', entropyA, 'b', entropyB, entropyA - entropyB
	...
	def gainRatio(self, examples, attribute, branchCount):
		...
		if intrinsicInformation == 0.: 
			#print "zero", intrinsicInformation
			#print "zero", examples.getExamples(values = True)
			return 0.
	...
	def bestAttribute(self, examples, attributes, branchCount):
		...
		scores = list()
		attributes = enumerate(attributes.attributes())
