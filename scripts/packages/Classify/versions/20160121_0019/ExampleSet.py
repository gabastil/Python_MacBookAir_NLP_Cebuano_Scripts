...
class ExampleSet(ObjectSet.ObjectSet):
	...
	def getExamples(self, label = None, values = False):
		...
		examples = [example for example in self.data]

		if label is not None:
			examples = [example for example in examples if example.getLabel() == label]
		
		if values:
			return [example.getValues()+[example.getLabel()] for example in examples]
