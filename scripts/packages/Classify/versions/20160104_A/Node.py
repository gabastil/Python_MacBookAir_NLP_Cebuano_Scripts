...
class Node(object):
	...
	def __init__(self, attributes = None, examples = None):
		self.node = self.grow(attributes, examples)
		self.data = None
		self.leaf = False

	def getBestAttribute(self, attributes, examples):
		""" get best attribute to split on based on gain ratio """
		infoGain = [self.getInformationGain(a, examples) 	  	for a in attributes.indices()]
		intrInfo = [self.getIntrinsicInformation(a, examples) 	for a in attributes.indices()]
		atScores = [self.getGainRatio(infoGain[a], intrInfo[a]) for a in attributes.indices()]
		return atScores.index(max(atScores))
	...
	def grow(self, attributes, examples):
		print attributes.indices(), attributes.get(2)
		print examples
		print self.getBestAttribute(attributes, examples)

	def OLD__grow(self, examples):
