...
class Node(ObjectSet.ObjectSet):
	...
	def gainRatio(self, examples, attribute, branchCount):
		...
		print "IG, II", informationGain, intrinsicInformation, [e.getValues() for e in examples]
	...
	def grow(self, examples, attributes, branchCount):
		...
		print examples.getAllLabels(),[e.getValues() for e in examples]
		...
		if self.homogenous(examples):
			...
			for e in split:
				print "Attribute: {0}\t{1}".format(best, [eexa.getValues() for eexa in e])
				if len(e) > 0:
					isNode = True
