...
class Node(ObjectSet.ObjectSet):
	...
	def getNumericBranches(self, examples, attribute, branchCount):
		...
		return [exampleRange[1] + (branchSlice*(j+1))for j in xrange(branchCount)]
	...
	def split(self, examples, attributes, best, branchCount):
		...
		for example in examples:
			v = example[best]
			#print "\nSplit(): ({1}) {0} \tIsNumeric: {2}".format(example, v, attribute.isNumeric())
			#print "Example: {0}\tLabel:{1}".format(example.getValues(), example.getLabel())

			if attribute.isNumeric():
				for i, b in enumerate(values):
					if b >= v:
						#print "Evaluating if v less than b", b,i, v, b >= v, values
						v = i
						break
